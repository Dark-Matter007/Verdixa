...
class Prompt3AdminApiTest {
 ...
 @BeforeEach void seed(){ User u=new User("prompt3user"+System.nanoTime(),"p"+System.nanoTime()+"@test.dev","hash",Role.USER);userId=users.save(u).getId(); Problem p=new Problem();p.setTitle("Prompt three problem "+System.nanoTime());p.setDescription("d");p.setDifficulty("EASY");p.setActive(true);problemId=problems.save(p).getId(); }
