...
export default function UserProfile() {
  ...
  return <UserShell context="Performance"><header className="vx-page-intro"><div><p className="vx-eyebrow">Developer performance</p><h1>Your work,<br/>made legible.</h1><p>Identity, consistency, and accepted outcomes—assembled from your persisted activity.</p><Link className="secondary-button vx-edit-profile-link" to="/profile/edit">Edit profile</Link></div>{progress && <div className="vx-profile-identity"><Avatar name={progress.username}/><div><strong>{progress.username}</strong><span>{progress.email}</span><small>{progress.role}</small></div></div>}</header>
    {error && <ErrorState message={error} onRetry={retry}/>} {!progress && !error && <LoadingState label="Building performance report"/>}
    {progress && <div className="vx-report-grid"><nav className="vx-report-nav" aria-label="Report sections"><a href="#overview">Overview</a><a href="#difficulty">Difficulty</a><a href="#activity">Activity</a><a href="#languages">Languages</a><Link to="/profile/certificates">Certificates</Link></nav><div>
      ...
      {analytics && <><section className="vx-report-section" id="activity"><span className="vx-section-meta">365 day signal</span><h2>Activity</h2><p className="vx-deck">{analytics.heatmap.activeDays} active days · {analytics.heatmap.currentStreak} day current streak · {analytics.heatmap.longestStreak} day longest streak</p><ActivityHeatmap days={analytics.heatmap.days || []}/></section><section className="vx-report-section" id="languages"><span className="vx-section-meta">Tooling</span><h2>Language usage</h2><div className="vx-language-list">{Object.entries(analytics.languageUsage || {}).map(([language,count]) => <div key={language}><strong>{language}</strong><span>{count} submissions</span></div>)}{!Object.keys(analytics.languageUsage || {}).length && <p>No persisted submissions yet.</p>}</div></section></>}
    </div></div>}
